...
const weatherPrompts = {
  getAverageTemps() {
    ...
    const result = weather.reduce((acc, place) => {
      acc.push((place.temperature.high + place.temperature.low) / 2);
      return acc;
    }, []);
    return result;
...
const ultimaPrompts = {
  totalDamage() {
    ...
    const result = characters.reduce((acc, character) => {
      acc += character.weapons.reduce((charAcc, weapon) => {
        charAcc += weapons[weapon].damage;
        return charAcc;
      }, 0);
      return acc;
    }, 0);
  ...
  charactersByTotal() {
    ...
    const result = characters.reduce((acc, character) => {
      acc.push({[character.name] : {
        damage: character.weapons.reduce((weaponAcc, weapon) => {
          weaponAcc += weapons[weapon].damage;
          return weaponAcc;
        }, 0),
        range: character.weapons.reduce((rangeAcc, weapon) => {
          rangeAcc += weapons[weapon].range;
          return rangeAcc;
        }, 0)
      }});
      return acc;
